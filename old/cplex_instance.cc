...
CplexInstance::CplexInstance(std::string model_name) {
    // do nothing
    importModel(model_name);
}

void CplexInstance::importModel(std::string model_name) {
    env = IloEnv();
...
bool CplexInstance::solve() {
    ...
    try {
        status = static_cast<bool>(cplex.solve());
    } catch (IloException& e) {
        std::cerr << "Concert exception caught: " << e << std::endl;
    }
...
Eigen::SparseVector<double> emitb() {
    return Eigen::SparseVector<double>();
}
Eigen::SparseVector<double> emitc() {
    return Eigen::SparseVector<double>();
}
        ...
        //std::cout << row_idx << " " << ma << " " << mb << std::endl;
...
// pair<string, double> next_variable(vector<string>::iterator& it,
// vector<string>::iterator enditer); bool rhs(string varname);
...
// if (varname == "=") {
//// duplicate constraint with swapped signs
// for (Triplet<double>& t: triplets_row) t = Triplet<double>(t.row()+1,
// t.col(), -t.value());
...
// pair<string, double> next_variable(vector<string>::iterator& it,
// vector<string>::iterator enditer) { if (it == enditer) return {"", 0.0};
